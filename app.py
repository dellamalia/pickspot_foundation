...
app = Flask(__name__)
app.secret_key = 'pickspot_s3cr3t_2025'
...
def home():
    ...
    cur = db.cursor()
    if lokasi:
        cur.execute("SELECT * FROM spots WHERE stok_booking > 0 AND lokasi LIKE %s ORDER BY rating DESC", (f'%{lokasi}%',))
    else:
...
def register():
    if request.method == 'POST':
        ...
        cur = db.cursor()
        try:
            cur.execute("INSERT INTO users (nama, email, password, role) VALUES (%s,%s,%s,'member')", (nama, email, password))
            db.commit()
...
def book_spot(id_spot):
    ...
    if spot and spot['stok_booking'] > 0:
        cur.execute("INSERT INTO bookings (id_user, id_spot, tanggal_waktu, status) VALUES (%s,%s,%s,'pending')", (session['id_user'], id_spot, datetime.now()))
...
def dashboard():
    ...
    cur.execute("SELECT s.*, COUNT(b.id_booking) AS jml_booking FROM spots s LEFT JOIN bookings b ON s.id_spot = b.id_spot GROUP BY s.id_spot ORDER BY s.id_spot DESC")
    spots = cur.fetchall()
    db.close()
    return render_template('dashboard.html', spots=spots, total_spots=total_spots, total_bookings=total_bookings, total_members=total_members)
...
def add_spot():
    if request.method == 'POST':
        data = (request.form['nama_spot'].strip(), request.form['deskripsi'].strip(), request.form['lokasi'].strip(), request.form['harga_estimasi'], request.form['stok_booking'])
        db  = get_db()
        cur = db.cursor()
        cur.execute("INSERT INTO spots (nama_spot, deskripsi, lokasi, harga_estimasi, stok_booking) VALUES (%s,%s,%s,%s,%s)", data)
        db.commit()
        db.close()
...
def edit_spot(id):
    ...
    cur = db.cursor()
    if request.method == 'POST':
        data = (request.form['nama_spot'].strip(), request.form['deskripsi'].strip(), request.form['lokasi'].strip(), request.form['harga_estimasi'], request.form['stok_booking'], id)
        cur.execute("UPDATE spots SET nama_spot=%s, deskripsi=%s, lokasi=%s, harga_estimasi=%s, stok_booking=%s WHERE id_spot=%s", data)
        db.commit()
        db.close()
...
def bookings_report():
    db  = get_db()
    cur = db.cursor()
    cur.execute("SELECT b.id_booking, u.nama AS nama_user, s.nama_spot, s.lokasi, b.tanggal_waktu, b.status FROM bookings b JOIN users u ON b.id_user=u.id_user JOIN spots s ON b.id_spot=s.id_spot ORDER BY b.tanggal_waktu DESC")
